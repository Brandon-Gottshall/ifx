...
ifx is an open-source (MIT & Public) project owned by Moons Out Labs, a legally declared DBA for Moons Out Media LLC (Ohio). Keep this document accurate as the repo evolves.

## Project Identity
- Name: ifx (lf control‑plane / extension CLI).
...
- Config is stored at `~/.config/ifx/config.toml` (TOML, user-editable source of truth).
...
- `internal/config/`: config loader for `~/.config/ifx/config.toml`.
...
- Ensure any network access is opt-in (`ifx icons set <pack>`), and document checksums in lockfiles.
