...
- `lfrc` modifications must be scoped to a clearly marked block (e.g., `# ifx:begin` / `# ifx:end`).
- CLI must never overwrite user-owned files outside its declared targets.
- Uninstall must fully remove only what ifx installed.
...
- `ifx doctor` (or equivalent) should report incomplete installs and suggest fixes.
