...
- Load config from `~/.config/ifx/config.toml`; warn on missing or invalid config before falling back to defaults.
- Add `lfx dev:tests` to run the MVP suite in a development environment.

Acceptance Tests:
- “ifx dev:tests runs the MVP suite in a development environment”
...
- “ifx doctor reports missing registry or lf config when absent”
...
- `brew tap <owner>/ifx && brew install ifx` succeeds on macOS.
