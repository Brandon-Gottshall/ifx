...
## Overview
ifx is a fast, pretty-by-default external control-plane CLI for lf. It manages themes, icons, and plugins by writing files into `~/.config/lf` while keeping `lf` itself untouched.
...
- No network calls unless explicitly required by a command (e.g., `ifx icons set <pack>`).
...
## Example Commands
- `ifx themes list`
- `ifx themes set catppuccin`
- `ifx icons list`
- `ifx icons set nvim-devicons`
- `ifx plugins install hotkeys-hud`
- `ifx plugins uninstall hotkeys-hud`
