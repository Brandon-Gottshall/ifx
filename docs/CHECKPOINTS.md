...
- Read `~/.config/ifx/config.toml` and warn on missing/invalid.
- Minimal surface area and clear failure modes.

## 3) Doctor Baseline
- `ifx doctor` checks registry presence and `~/.config/lf` existence.
...
## 7) Dev Test Harness
- `ifx dev:tests` to validate MVP behaviors.
